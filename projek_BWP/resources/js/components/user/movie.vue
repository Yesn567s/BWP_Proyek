...
onMounted(async () => {
  try {
    ...
    nowPlayingItems.value = nowPlayingRes.data
    upcomingItems.value = upcomingRes.data
  } catch (err) {
...
const filteredNowPlaying = computed(() =>
  filterMovies(nowPlayingItems.value)
)

const filteredUpcoming = computed(() =>
  filterMovies(upcomingItems.value)
)
</script>
...
<template>
    ...
    <div class="row g-4">
      <div
        v-for="item in filteredNowPlaying"
        :key="item.id"
